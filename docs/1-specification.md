...
5. If registered: embeds the device index as a JavaScript constant (`const DEVICE_IDX = N`), fetches `GET /api/dyn?device_idx=N` to populate an activity button list, and renders a launch page.  Clicking a button navigates to `/dyn_activities/<name>?pin=<PIN>&device_idx=N&act_id=<id>&credits_s=<credits>`.
...
2. Searches `g_dyn_act_registry[]` for an entry whose `p_name` matches.
3. On match: serves the embedded blob as `text/html`.
4. On no match: HTTP 404.
...
`403 Forbidden` when neither admin credentials nor a valid PIN are supplied;
...
2. The firmware detects the device automatically; a list of assigned mini-games is shown.
3. Tap a game to open it; play and click "Claim Credits" to earn internet time.
4. The mini-game POSTs to `/api/activities/credit` with the device-bound PIN (injected by the `/dyn` page as a URL query parameter).
