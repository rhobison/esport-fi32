...
This should allow in the future that certain activities are like games, where the user gain more credits the faster he/she completes the activity. For example, we can have math game, where the kid should solve simple multiplication/adition/subtraction calculations faster in order to earn the credits. The faster the solution, the more credits gained. -- planned
8. Implementation of the infrastructure for dynamic_activities (activities that are like mini-games).
- In order to make it possible for the kid to access/call the API to credit the internet time at the end of each dynamic_activity, a rudimentary 'authentication' should be in place. The API must be changed to include the a fild called 'pin'. This field must be filled automatically by the device as the some kind of the hash of its MAC address. This ensures the kid can only place credit for himself. The backend can check this hashed pin because it has the MAC of the device.
- The dynamic_activities will be implemented as html files in the directory `main/dyn_activities`. Those html files must be embedded as binary blobs into the FW using `embed_files` or `idf_component_register`.
- A new field `dynamic activity` should be added to the activity fields. It must be shown as a checkbox when inserting a new activity in `/activities/manage`. When the checkbox is checked, the textbox for activity name should turn into a combobox listing the dynamic activities embedded into the fw (maybe we should filter the ones that are already inserted for that user).
- When inserting a new activity in `activities`, the credit field must be validated, it cannot be 0 anynmore.
