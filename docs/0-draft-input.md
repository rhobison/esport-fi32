...
7. Besides the bike, the system should allow an adult/admin to give internet credits to a given user based on a list of activities. The activities are pre-registered/added via a dedicated activity manager page. Each activity should have:
   ...
   - a name/description (max 20 characters)
   - how much credit the user/device earn when a given activity is completed. This is internet time in seconds, but displayed as h:mm:ss, with autoformat the same way as the counter field in the device registration.
   - a boolean/checkbox indicating if the activity can be done just once a day.
It should be possible to associate/insert activities to a given user/registered device (the users are the same we insert via config page). It should be possible to associate/insert up to 20 activities to each user.
...
For each activity for the user, a button on the left side of the activity with the text "Credit", when clicked/pressed should add the corresponding credits to the user counter (the credits on the side of the combobox should reflect the added credits). If the activity is 'once a day' and the credits were already credited on the current day, the button should be grayed/disabled.
The log of the last 30 credited activities should be displayed below the list of activities, per user (the log should be shown when the user is selected and updated when an activity is credited).
