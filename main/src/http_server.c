    ...
    size_t out_len)
{
    ...
    while (NULL != p_pos)
    {
        if ((0 == strncmp(p_pos, p_key, key_len)) && ('=' == p_pos[key_len]))
        {
            ...
            /* Cap the raw length to protect the intermediate stack buffer. */
            char raw[HTTP_SRV_FORM_VALUE_ENC_MAX_LEN];
...
static esp_err_t http_srv_config_get_handler(httpd_req_t * p_req)
{
     ...
     */
    char enc[HTTP_SRV_ATTR_ENC_LEN];
...
static esp_err_t http_srv_config_post_handler(httpd_req_t * p_req)
{
    ...
    char body[HTTP_SRV_POST_BODY_MAX_LEN];
    int  total_read = 0;
...
static esp_err_t http_srv_api_sessions_daily_handler(httpd_req_t * p_req)
{
    ...
    http_srv_daily_bin_t bins[HTTP_SRV_DAILY_WINDOW_DAYS];
